...
pyngp_path = '/home/kev/instant-ngp/build'
...
if __name__ == "__main__":
	...
	output_dir = "/home/kev/instant-ngp/data/nerf/fox/output"  # Cambia a tu ruta de salida
	os.makedirs(output_dir, exist_ok=True)  # Crea el directorio si no existe
	output_txt = os.path.join(output_dir, "nerf_gui_params.txt")  # Define la ruta completa del archivo
		...
		with tqdm(desc="Training", total=n_steps, unit="steps") as t:
			...
			while testbed.frame():
				with open(output_txt, 'w') as f:
					f.write("Captured NeRF Parameters from GUI:\n")
					f.write(f"Scene Path: {args.scene}\n")
					f.write(f"Network Config: {args.network}\n")
					f.write(f"Training Steps Target: {n_steps}\n")
					f.write(f"Current Training Step: {testbed.training_step}\n")
					f.write(f"Exposure: {testbed.exposure}\n")
					f.write(f"Sharpening: {testbed.nerf.sharpen}\n")
					f.write(f"Resolution: {args.width}x{args.height}\n")
					f.write(f"Near Distance: {testbed.nerf.training.near_distance}\n")
					f.write(f"NeRF Compatibility Mode: {'Enabled' if args.nerf_compatibility else 'Disabled'}\n")
					f.write(f"Marching Cubes Resolution: {args.marching_cubes_res}\n")
					f.write(f"Marching Cubes Density Threshold: {args.marching_cubes_density_thresh}\n")
					f.write(f"Current Loss: {testbed.loss}\n")
					f.write(f"GUI Mode: {'Enabled' if args.gui else 'Disabled'}\n")
					f.write(f"Save Mesh Path: {args.save_mesh}\n")
					
					# Capturar parámetros internos del testbed para replicar en el script
				
					f.write(f"Background Color: {testbed.background_color}\n")
					f.write(f"Tonemap Curve: {testbed.tonemap_curve}\n")
					f.write(f"Render With Lens Distortion: {testbed.nerf.render_with_lens_distortion}\n")
					f.write(f"Random Background Color: {testbed.nerf.training.random_bg_color}\n")
					f.write(f"Cone Angle Constant: {testbed.nerf.cone_angle_constant}\n")
	...
	# 	testbed.compute_and_save_marching_cubes_mesh(args.save_mesh, [128, 128, 128], ngp.BoundingBox([0.318, 0.237, 0.321], [0.670, 0.391, 0.754]))

	if args.save_mesh:
		res = args.marching_cubes_res or 256
		thresh = args.marching_cubes_density_thresh or 2.5
		output_txt = args.save_mesh.replace('.obj', '_params.txt')

		print(f"Generating mesh via marching cubes and saving to {args.save_mesh}. Resolution=[{res},{res},{res}], Density Threshold={thresh}")
		
		# b_min = np.array([0.25, -1.5, -1.5],)
		# b_max = np.array([1.5, 2.5, 2.5])
		...
		# 	args.save_mesh, 
		# 	[res, res, res], 
		# 	ngp.BoundingBox([0.318, 0.237, 0.321], [0.670, 0.391, 0.754])
		# )
		...
		# testbed.render_aabb = ngp.BoundingBox([0.25, -1.5, -1.5], [1.5, 2.5, 2.5])
		...
		with open(output_txt, 'w') as f:
			...
			f.write(f"Density Threshold: {thresh}\n")
			f.write(f"Scene Path: {args.scene}\n")
			f.write(f"Network Config: {args.network}\n")
			f.write(f"Training Steps: {args.n_steps}\n")
			f.write(f"Exposure: {args.exposure}\n")
			f.write(f"Sharpening: {args.sharpen}\n")
			if args.nerf_compatibility:
				f.write(f"NeRF Compatibility Mode: Enabled\n")
			else:
				f.write(f"NeRF Compatibility Mode: Disabled\n")
			f.write(f"Near Distance: {args.near_distance}\n")

		print(f"Mesh parameters saved to {output_txt}")
